...
from mehr_crawler.items import MehrCrawlerItem


class MehrNewsSpider(CrawlSpider):
    name = 'Mehr'
    ...
    for yr in range(year_start, year_end + 1):
        for mn in range(month_start, month_end + 1):
            for dy in range(day_start, day_end + 1):
                url = f"https://www.mehrnews.com/archive?pi=1&ms=0&dy={dy}&mn={mn}&yr={yr}"
    ...
    def parse_news(self, response):
        ...
        item['category'] = response.xpath('//ol[@class="breadcrumb"]//li/a/text()').get().strip()
